...
/**
  ...
  8.关于Student的数组的内存解析可以再看一遍（对象数组的内存解析P200）引用类型的变量只能存地址值或者是null，地址值只能存一份
      -->Student stu [] = new Student[5] stu是引用类型变量只能存地址或者是null
  ...
  14.如果参数是引用数据类型，此时实参赋给形参的是实参真实存储的地址值。两份值共享。p211
       方法执行完后变量会弹出栈 此时变量存的地址值的指向也没有了，堆里的对象就成了垃圾等待回收
  15.方法的重载和权限修饰符、返回值类型、形参变量名、方法体都没关系
  16.引用数据类型一般都是new出来 除了String 。 例如:String str="Hello"。另外String 在值传递的时候也很特殊。P218
     总结就一句话 值传递机制就是存什么传什么
     String比较特殊是因为String在常量池中而且底层是不可修改的char数组，就算其中一个变量改了，也是新造一个数组，而不是对之前那的数组修改
     P218 还可以再看
     emmm，好像基本类型和String不能交换 其他的引用类型可以交换成功
